...
namespace E_Project_3_API.Services
{
    public class AuthenticationServices : IAuthentication
    {
        ...
        public RegisterResponse Register(UserRegisterDto request)
        {
            ...
            var existedUser = _dataContext.Set<User>().ToList().SingleOrDefault(u => u.Email == request.Email && u.Role == true);
        ...
        public LoginResponse Login(UserLoginDto request)
        {
            ...
            var foundUser = _dataContext.Set<User>().ToList().SingleOrDefault(u => u.Email == request.Email);
            ...
            else
            {
                if (request.Email == "")
                {
                    loginResponse.Errors.EmailEmptyError = "Email is required";
                }
                if (request.Password == "")
                {
