...
namespace E_Project_3_API.Services
{
    public class ShowtimeServices : IShowtimeServices
    {
        ...
        public List<ShowtimeResponse> GetShowtimesByMovieAndDate(int movieId, int dateId)
        {
            var tickets = from m in _dataContext.Movies
                          join t in _dataContext.Tickets on m.Id equals t.Movie.Id
                          where t.Movie.Id == movieId && t.Date.Id == dateId
                          orderby t.Showtime.StartTime.Hour ascending
                          select new
                          {
                              TicketId = t.Id
                          };
